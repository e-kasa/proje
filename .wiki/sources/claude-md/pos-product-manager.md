---
title: CLAUDE.md — pos-product-manager
type: source
source: pos-product-manager/CLAUDE.md
ingested: 2026-04-25
...
Şablonlar: `.wiki/syntheses/runbook-new-entity.md`, `.wiki/syntheses/runbook-new-endpoint.md`.  
Multi-tenant: `.wiki/concepts/multi-tenant-routing.md`. API zarfı: `.wiki/concepts/api-response.md`.
...
Controller `/api/v1/...` yazar — Flutter `product/api/v1/...` çağırır. Detay: `.wiki/concepts/url-routing.md`.
...
`CompanySettingServiceImpl.updateSettings()` → `sectorType` güncellenmez (kurulumda sabit). Detay: `.wiki/concepts/sector-strings.md`.
...
Exception → HTTP tablosu: `.wiki/concepts/api-response.md`.
...
`.wiki/sources/status-snapshots/sprint.md` → Sprint 1 checklist.
