...
class WizardState extends ChangeNotifier {
  ...
  // ─── Save & Continue Helpers ──────────────────────────────────────────────

  RetainedFields captureRetainedFields() {
    return RetainedFields(
      ...
      taxExempt: taxExempt,
      specialTaxRate: specialTaxRateController.text,
      withholdingTaxRate: withholdingTaxRateController.text,
      supplierId: selectedSupplier,
      selectedStores: List.from(selectedStores),
      selectedWarehouses: List.from(selectedWarehouses),
  ...
  void applyRetainedFields(RetainedFields retained) {
    selectedCategory = retained.categoryId;
    selectedUnit = retained.selectedUnit;
    selectedVatRate = retained.selectedVatRate;
    vatIncluded = retained.vatIncluded;
    taxExempt = retained.taxExempt;
    if (retained.specialTaxRate != null && retained.specialTaxRate!.isNotEmpty) {
      specialTaxRateController.text = retained.specialTaxRate!;
    }
    if (retained.withholdingTaxRate != null && retained.withholdingTaxRate!.isNotEmpty) {
      withholdingTaxRateController.text = retained.withholdingTaxRate!;
    }
    selectedSupplier = retained.supplierId;
    selectedStores = List.from(retained.selectedStores);
    selectedWarehouses = List.from(retained.selectedWarehouses);
    brandController.text = retained.brandText;
    notifyListeners();
  }
  ...
  void dispose() {
    ...
    specialTaxRateController.dispose();
    withholdingTaxRateController.dispose();
    basePriceController.dispose();
    basePurchasePriceController.dispose();
